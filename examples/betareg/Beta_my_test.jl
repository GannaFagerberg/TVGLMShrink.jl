...
@views function condMean_fixedμ(param, state, t)
    ...
    # Known Beta mean for observations in group t
    μ = μ_fixed_groups[t]

    # First and only state is log-precision
...
@views function condCov_fixedμ(param, state, t)
    ...
    # Variance of log(Y) under the Beta model
...
quant_paramtime_iplf =
    quantile_multidim(θpost, [0.025, 0.5, 0.975], dims=3)

# θpost[:, 1, :] is now γₜ = log(κₜ)
plt_iplf = plot(
    dateVec,
    γ[:, 1];
    label="True",
