# Simulators and utilities for the Poisson regression model

function simulateDSP(T, p, μ, φ, α, β, X, link, FisherInfoPois; initval=zeros(p)')
    y = zeros(T)
    λtime = zeros(T)
    θ = [initval; zeros(T, p)]
    h = [μ'; zeros(T, p)]
    ...
    param = TVGLMmodel(LogVol2Covs(zeros(1, p)), [[X]], [X], link, [1:p])

    for t in 2:(T+1)

        S = inv(sqrt(FisherInfoPois(param, θ[t-1, :], t - 1) / T))
        κ = rand(Beta(β, α), p)
        η_t = log.(1 ./ κ .- 1)
    ...
    for i in 1:T
        λtime[i] = linkinv.(link[1], dot(θ[i+1, :], X[i, :]))
...
function simulate_poisson_reg_data(T, p, covSel, link, Φ, Σₑ, mₑ)
    X = ones(T + 1)
    X = hcat(X, simulateVAR(T + 1, Φ, Σₑ, mₑ))
    X = X[2:end, :]
    β, y, λtime = simulateDSP(T, p, [-15, -15, -15], 0.5, 1 / 2, 1 / 2,
        X, link, FisherInfoPois)
...
function simulate_poisson_reg_data_fixed_together(T, p, covSel, link, φ, σₑ, mₑ, β₀)
    X = ones(T + 1)
    X = hcat(X, simulateVAR(T + 1, [φ], σₑ, mₑ))
