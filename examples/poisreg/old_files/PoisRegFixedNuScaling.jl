...
Random.seed!(slurm_id + 1); # set seed for reproducibility, different seed for each slurm_id
applName = "PoisRegFixedNu" # name for saving results and figures
...
φ = 0.7
y, X, β, λtime = simulate_poisson_reg_data_fixed_innov(T, p, X, covSel, invlink_dgp,
    FisherInfo, φ, σₑ, mₑ; initval=zeros(p))
...
## Laplace approximation - no scaling
methodlabel = "Laplace_N"
scaling = :none
...
# Parameter quantiles on the parameter time scale - this always includes t=0
quant_paramtime_ln = quantile_multidim(θpost, [0.025, 0.5, 0.975], dims=3);

PlotPostParamEvolution!(plt, quant_paramtime_ln, methodlabel,
    groupSizes; dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:solid, lw=2, c=colors[3])

## Laplace approximation - diag scaling
methodlabel = "Laplace_D"
scaling = :diagonal
algoSettings = (; algoSettings..., scaling=scaling, stateSamplingMethod=:ffbs_laplace);
dataSettings = (y=y, X=X, covSel=covSel, nPerGroup=nPerGroup);
...
# Parameter quantiles on the parameter time scale - this always includes t=0
quant_paramtime_ld = quantile_multidim(θpost, [0.025, 0.5, 0.975], dims=3);

PlotPostParamEvolution!(plt, quant_paramtime_ld, methodlabel,
    groupSizes; dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:solid, lw=2, c=colors[2])


## Laplace approximation - full scaling
methodlabel = "Laplace_F"
scaling = :full
algoSettings = (; algoSettings..., scaling=scaling, stateSamplingMethod=:ffbs_laplace);
dataSettings = (y=y, X=X, covSel=covSel, nPerGroup=nPerGroup);
...
# Parameter quantiles on the parameter time scale - this always includes t=0
quant_paramtime_lf = quantile_multidim(θpost, [0.025, 0.5, 0.975], dims=3);

PlotPostParamEvolution!(plt, quant_paramtime_lf, methodlabel,
    groupSizes; dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:solid, lw=2, c=colors[4])

savefig(figFolder * "$(applName)_param_scaling_$(dataSettings.nPerGroup).svg")
savefig(figFolder * "$(applName)_param_scaling_$(dataSettings.nPerGroup).pdf")
