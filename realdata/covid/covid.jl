...
includet(joinpath(@__DIR__, "../..") * "/examples/poisreg/PoisModel.jl") # BetaReg stuff
...
## observation data
applName = "covid" # all save files with this prefix
df_ICU = CSV.read(dataFolder * "/ICUCasesUppsala.csv", DataFrame)
df_wastewater = CSV.read(dataFolder * "/wastewater.csv", DataFrame)

df = innerjoin(df_ICU, df_wastewater, on=:date)
sort!(df, :date)
...
plt = PlotPostParamEvolution(quant_paramtime,
    "PGAS",
    groupSizes; titles=titles, dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:shaded, lw=1, c=colors[1])
...
μmedian = zeros(T)
ϕmedian = zeros(T)
for t in 1:size(quant_obstime, 1)
    μmedian[t] = invlinkmean(X[t, covSel[1]]' * quant_obstime[t, 1:p, 2])
    ϕmedian[t] = invlinkprec(X[t, covSel[2]]' * quant_obstime[t, p+1:end, 2])
end
pdens = plot_betadensity_fit(μmedian, ϕmedian, y; datevec=dateVec)
savefig(figFolder * "$(applName)_dens_$(methodlabel)_$(algoSettings.scaling)_$(dataSettings.nPerGroup).svg")
pdens_zoom = plot_betadensity_fit(μmedian, ϕmedian, y; datevec=dateVec, timePoints=300:432)
...
PlotPostParamEvolution!(plt, quant_paramtime,
    "Laplace",
    groupSizes; dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:solid, lw=1, c=colors[3])
plot(plt...)
...
PlotPostParamEvolution!(plt, quant_paramtime,
    "IPLF",
    groupSizes; dateVec=dateVec, interpMethod=interpMethod, plot_t0=keep_t0, interval_style=:solid, lw=1, c=colors[2])
plot(plt...)
